...
puts "\nStarting Demo 24: Setting OpenFlow flow on the Controller: "\
...
pop_ether_type = 34887
output_port = 13

puts "\nMatch: Ethernet Type 0x#{eth_type.to_s(16)}, MPLS Label 0x#{mpls_label.to_s(16)} "\
  "In port #{in_port}"
puts "Action: Pop MPLS (Ethernet Type #{pop_ether_type}, "\
  "Output (Physical Port Number #{output_port})"

sleep(delay)
flow_id = 29
table_id = 0
flow_entry = FlowEntry.new(flow_priority: 1023, flow_table_id: table_id,
  flow_id: flow_id, name: "Strip MPLS Label", cookie: 889, cookie_mask: 255)
# Instruction: 'Apply-action'
#     Actions: 'Pop MPLS Header'
#              'Output'
instruction = Instruction.new(instruction_order: 0)
action = PopMplsHeaderAction.new(order: 0, eth_type: pop_ether_type)
