...
config = YAML.load_file('sample_commands/config.yml')
...
puts "\nAdding #{node.name} to the controller #{controller.ip}"
response = controller.add_netconf_node(node)
if response.status == NetconfResponseStatus::OK
  puts "#{node.name} was successfully added to the controller"
else
